...
namespace DotLiquid.Tests
{
    public class FunctionFilterTests
    {
        ...
        {
            _context["var"] = 2;
            _context.AddFilter<int, string>("add_two", i => (i + 2).ToString(CultureInfo.InvariantCulture));
        ...
        public void AddingAnonimousFunctionWithClosure()
        {
            ...
            _context.AddFilter<int, string>("add_two", i => (x=(i + x)).ToString(CultureInfo.InvariantCulture));
            Assert.That(new Variable("var | add_two").Render(_context), Is.EqualTo("4"));
